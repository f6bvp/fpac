...
static void close_client(int client, int active)
{
	...
	/* P6 F6BVP 2026-06-13: close_client() must be idempotent. poll_adjacents()
	 * (P1) and the cross-connection paths can call it on an fd already closed
	 * (context[]==NULL) or on a reset adjacent (wpa->context==-1), which
	 * crashed on context[client]->dirty_list (NULL+0x48) and inside libax25
	 * (rose_ntoa on a NULL context). Guard against both. */
	if (client < 0 || !context[client]) return;
 ...
 */
	{
	wp_t chk_wp;
	if (pdu.type == wp_type_set && pdu.data.wp.is_deleted && (db_get(&pdu.data.wp.address.srose_call, &chk_wp) != 0))
		{
		if (verbose) syslog(LOG_INFO, "rose_read_handler() received deleted WP record '%s' from adjacent %s ignored (unknown callsign)",
		ax25_ntoa(&pdu.data.wp.address.srose_call),
		rose_ntoa(&context[s]->address.srose_addr));
		again = (rc == 2);   /* P3 F6BVP 2026-06-12: skip, keep session up */
		continue;
		}
	}
	  ...
	  switch (pdu.type) {
	  case wp_type_set:
		/* P2 F6BVP 2026-06-12: obsolescence policy moved here from the wire
		 * codec (libwp.c). Skip an obsolete deleted record WITHOUT closing the
		 * session (the codec used to return -1 -> close_client). Mirrors the
		 * send-side skip in rose_write_handler. */
		if (pdu.data.wp.is_deleted && ancien(&pdu)) {
			if (verbose) syslog(LOG_INFO, "Ignoring obsolete deleted record %s from %s",
				ax25_ntoa(&pdu.data.wp.address.srose_call),
				rose_ntoa(&context[s]->address.srose_addr));
			break;
		}
...
static void poll_adjacents(void)
{
	...
	for (wpa=wp_adjacent_list; wpa; wpa=wpa->next) {
		...
		case WPA_CONNECTED:
			...
			break;
		case WPA_CONNECTING:
			/* P1 F6BVP 2026-06-12: time out a hung non-blocking ROSE connect.
			 * poll_adjacents only retries DISCONNECTED, so a connect stuck in
			 * WPA_CONNECTING (no WRITE_EVENT, no fd error - e.g. a ROSE call
			 * request that never completes nor times out) would isolate the
			 * node until a manual fpacwpd restart. retry_connect_date was
